<template>
    <div @click.prevent="handleEditTask(task)" style="cursor: pointer" class="">
        <div class="card">
    ...
    <div class="modal edit-task-modal" :class="[modalEditTask ? 'is-active': '']">
        ...
        <div @click="switchEdit(false)" class="modal-content">
            <div style="margin-bottom: -10px;" class="tile is-vertical notification" 
            :class="taskColor">
                <p  v-show="toEdit != true" class="title is-2 no-margin">{{task.title}}</p>
                <input v-show="toEdit == true" v-model="taskToEdit.title" class="input is-large" type="text" placeholder="Task title">
                <p class="subtitle is-size-6">{{task.completed ? 'Completed' : 'In-progress'}}</p>
            ...
            <div class="box">
                <div class="block content">
                    ...
                    <div :style="[toEdit ? {'visibility': 'hidden'} : {}]" class="is-flex is-justify-content-space-between mt-4">
